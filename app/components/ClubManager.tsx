...
export default function ClubManager({ userRole }: { userRole: string }) {
  ...
  const [studentForm, setStudentForm] = useState<{ full_name: string; dob: string; belt_level: string | number; join_date: string }>({ 
      full_name: '', dob: '', belt_level: 0, join_date: '' 
  });
  const [upgradeForm, setUpgradeForm] = useState({ studentId: '', email: '', password: '', fullName: '' });
  
  const [targetRole, setTargetRole] = useState(''); 
  const [searchTerm, setSearchTerm] = useState(''); // Tìm kiếm trong Modal gán HLV
  const [memberFilter, setMemberFilter] = useState(''); // Tìm kiếm thành viên
  const [clubSearchTerm, setClubSearchTerm] = useState(''); // ✅ MỚI: Tìm kiếm CLB
  
  const [isEditingStudent, setIsEditingStudent] = useState(false);
  const [editingStudentId, setEditingStudentId] = useState<string | null>(null);

  const isAdmin = userRole === 'admin' || userRole === 'master_head' || userRole === 'grandmaster';
  const isCoach = userRole === 'instructor' || isAdmin; 
  ...
  const handleSaveStudent = async (e: React.FormEvent) => {
    e.preventDefault(); if (!selectedClub) return; setLoading(true);
    const url = isEditingStudent ? '/api/admin/update-user' : '/api/admin/create-user';
    const body: any = { fullName: studentForm.full_name, dob: studentForm.dob, join_date: studentForm.join_date, belt_level: Number(studentForm.belt_level) || 0 };
  ...
  const handleDeleteStudent = async (studentId: string, name: string) => { if(!confirm(`Xóa võ sinh "${name}"?`)) return; const res = await fetch('/api/admin/delete-user', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: studentId }), }); const result = await res.json(); if(result.success) setStudents(prev => prev.filter(s => s.id !== studentId)); else alert(result.error); }
  const handleAssignCoach = async (coach: Member) => { if (!selectedClub) return; setLoading(true); const res = await fetch('/api/admin/update-user', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: coach.id, club_id: selectedClub.id, club_role: targetRole }), }); setLoading(false); const result = await res.json(); if (result.success) { setShowAssignModal(false); const { data } = await supabase.from('profiles').select('*').eq('club_id', selectedClub.id); if (data) { const all = data as any as Member[]; setStaffs(all.filter(m => m.club_role || m.role !== 'student')); } } else alert(result.error); };
    ...
    <div className="flex flex-col md:flex-row h-full w-full bg-stone-100 overflow-hidden relative">
      <div className="flex-1 flex flex-col min-w-0 bg-stone-50">
          ...
          <div className="h-14 bg-white border-b border-gray-300 shadow-sm flex items-center px-4 gap-4 z-10 shrink-0 overflow-x-auto">
              ...
              <div className="flex gap-2">
                  {regions.map(r => (
                      <button 
                        key={r.id} 
                        onClick={() => { setSelectedRegionName(r.name); setSelectedClub(null); setClubSearchTerm(''); }} 
                        className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all whitespace-nowrap border ${selectedRegionName === r.name ? 'bg-red-900 text-yellow-50 border-red-900 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:border-red-300 hover:text-red-800'}`}
                      >
                          {r.name}
                      </button>
                  ))}
          ...
          <div className="flex-1 flex overflow-hidden">
              ...
              {/* --- CỘT TRÁI: DANH SÁCH CLB + TÌM KIẾM --- */}
                  ...
                  <div className="p-3 bg-gray-50 border-b flex flex-col gap-3">
                      ...
                      <div className="relative">
                          <input 
                              ...
                              className="w-full pl-8 pr-2 py-1.5 rounded border border-gray-300 bg-white text-xs focus:border-red-800 focus:ring-1 focus:ring-red-800 outline-none transition-all"
                      ...
                      <div className="max-w-5xl mx-auto space-y-8 pb-20">
                          ...
                          <div className="bg-white p-6 rounded-lg shadow-md border-t-4 border-red-800 flex flex-col md:flex-row justify-between items-center gap-4">
                              ...
                              <div className="relative w-full md:w-64">
                                  <input 
                                      ...
                                      className="w-full pl-9 pr-4 py-2 rounded-full border border-gray-200 bg-gray-50 focus:border-red-800 focus:bg-white outline-none text-sm transition-all"
                                      value={memberFilter}
                          ...
                          <div>
                              <div className="flex justify-between items-end mb-4 border-b pb-2">
                                  ...
                                  {isCoach && <button onClick={() => { setIsEditingStudent(false); setStudentForm({full_name: '', dob: '', belt_level: 0, join_date: ''}); setShowStudentModal(true); }} className="bg-red-800 hover:bg-red-700 text-white px-4 py-2 rounded shadow font-bold text-sm flex items-center gap-1">+ Thêm Võ Sinh</button>}
                              </div>
                              <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                                  <table className="w-full text-sm text-left">
                                      ...
                                      <tbody className="divide-y divide-gray-100">
                                          {filteredStudents.map(st => (
                                              <tr key={st.id} className="hover:bg-red-50 transition-colors group">
                                                  ...
                                                  <td className="px-4 py-3 text-right flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                                      {isCoach && <><button onClick={() => { setIsEditingStudent(true); setEditingStudentId(st.id); setStudentForm({full_name: st.full_name, dob: st.dob || '', belt_level: st.belt_level, join_date: st.join_date || ''}); setShowStudentModal(true); }} className="text-blue-600 hover:bg-blue-50 px-2 py-1 rounded text-xs font-bold border border-transparent hover:border-blue-200">Sửa</button><button onClick={() => handleDeleteStudent(st.id, st.full_name)} className="text-red-600 hover:bg-red-50 px-2 py-1 rounded text-xs font-bold border border-transparent hover:border-red-200">Xóa</button></>}
                                          ...
                                          {filteredStudents.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-gray-400 italic">Chưa có dữ liệu võ sinh (hoặc không tìm thấy).</td></tr>}
                                      </tbody>
                                  </table>
                              </div>
                          </div>
      ...
      {showStudentModal && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
              <form onSubmit={handleSaveStudent} className="bg-white p-6 rounded shadow-lg w-96 border-t-4 border-red-900 animate-in zoom-in duration-200">
                  ...
                  <div className="flex justify-end gap-2 mt-6"><button type="button" onClick={() => setShowStudentModal(false)} className="px-3 py-1 text-gray-500 hover:bg-gray-100 rounded">Hủy</button><button disabled={loading} className="px-4 py-1 bg-red-900 text-white rounded font-bold shadow">{loading ? '...' : 'Lưu'}</button></div>
              </form>
          </div>
      )}
              ...
              <div className="bg-white rounded-lg shadow-xl w-full max-w-md h-[500px] flex flex-col animate-in zoom-in duration-200">
                  ...
                  <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                      {filteredCoachesInModal.map((c: any) => (
                          <div key={c.id} onClick={() => handleAssignCoach(c)} className="flex items-center gap-3 p-2 hover:bg-red-50 cursor-pointer rounded border border-transparent hover:border-red-200 transition-colors">
