...
export default function Header() {
  ...
  const checkUser = async () => {
    // Dùng getUser() thay vì getSession() để tránh lỗi property 'session'
  ...
  const handleUserAvatarChange = (e: any) => {
      const file = e.target.files[0];
      if (file) {
  ...
  const handleSaveUserProfile = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (userAvatarFile) {
              ...
              const { error } = await supabase.storage.from('assets').upload(fileName, userAvatarFile);
              if (error) throw error;
          ...
          const res = await fetch('/api/profile/update', {
              ...
              body: JSON.stringify({ ...userForm, avatar_url: avatarUrl }),
          });
    ...
    <header className="bg-red-900 text-yellow-50 shadow-md border-b-4 border-yellow-600 py-2 px-3 md:py-3 md:px-6 flex justify-between items-center relative z-50 min-h-[60px]">
      <div className="flex items-center gap-2 md:gap-4 flex-1">
        ...
        <div className="flex-1">
            <div className="flex items-start gap-1">
                <h1 className="text-sm md:text-3xl font-serif font-bold text-yellow-50 drop-shadow-sm uppercase break-words leading-tight">
                    {info.name}
                </h1>
                {isAdmin && (
            ...
            <div className="bg-white p-6 rounded-lg w-full max-w-md border-4 border-yellow-600 relative shadow-2xl animate-in zoom-in duration-200">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input value={editName} onChange={e => setEditName(e.target.value)} className="w-full border-2 border-red-900/20 p-3 rounded focus:border-red-900 outline-none font-serif text-lg text-red-900"/>
                    </div>
                    <button onClick={handleUpdateName} disabled={uploading} className="w-full bg-red-900 text-white font-bold py-3 rounded hover:bg-red-800 shadow mt-2">
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[110] p-4 backdrop-blur-sm">
            <div className="bg-[#fdfbf7] rounded-2xl shadow-2xl w-full max-w-md p-6 border-4 border-double border-red-900/20 animate-in zoom-in duration-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
                <div className="flex justify-between items-center mb-4 border-b border-red-900/10 pb-2">
                    <h3 className="text-xl font-serif font-bold text-red-900 uppercase">Hồ Sơ Cá Nhân</h3>
                    <button onClick={() => setShowUserModal(false)} className="text-stone-400 hover:text-red-900 text-2xl font-bold">&times;</button>
                </div>
                <form onSubmit={handleSaveUserProfile} className="space-y-4">
                    <div className="flex flex-col items-center gap-2">
                        <div className="relative w-24 h-24 rounded-full border-2 border-red-900 overflow-hidden group shadow-md bg-white">
                            <img src={userAvatarPreview || userForm.avatar_url || '/default-avatar.png'} className="w-full h-full object-cover" />
                    ...
                    <div>
                        <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Họ và Tên</label>
                        <input required className="text-red-900 font-bold w-full border border-stone-200 p-2 rounded-lg bg-white outline-none focus:border-red-800" 
                    ...
                    <div>
                        <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Ngày Sinh (Dương Lịch)</label>
                        <input type="date" className="text-red-900 font-medium w-full border border-stone-200 p-2 rounded-lg bg-white outline-none focus:border-red-800 cursor-pointer" 
                            value={userForm.dob || ''} 
                    ...
                    <div>
                        <label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Giới thiệu (Bio)</label>
                        <textarea rows={4} className="text-stone-700 w-full border border-stone-200 p-2 rounded-lg bg-white outline-none focus:border-red-800 text-sm resize-none" 
                            placeholder="Tiểu sử, thành tích, quá trình tập luyện..." 
                            value={userForm.bio} 
                            onChange={e => setUserForm({...userForm, bio: e.target.value})} 
                        />
                    </div>
                    
                    <div className="flex justify-end gap-3 pt-4 border-t border-red-900/10">
                        <button type="button" onClick={() => setShowUserModal(false)} className="px-4 py-2 text-stone-500 font-bold hover:bg-stone-100 rounded-lg text-sm">Hủy</button>
                        <button type="submit" disabled={uploading} className="px-6 py-2 bg-red-900 text-white rounded-lg font-bold shadow-md hover:bg-red-800 text-sm">
                            {uploading ? 'Đang lưu...' : 'Lưu Hồ Sơ'}
                        </button>
                    </div>
                </form>
            </div>
