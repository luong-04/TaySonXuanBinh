...
export default function CoachManager({ userRole }: { userRole: string }) {
  ...
  // --- XÓA HLV VĨNH VIỄN (XÓA CẢ AUTH) ---
  const handleDelete = async (id: string, name: string) => {
      if (!confirm(`CẢNH BÁO: Bạn có chắc muốn XÓA VĨNH VIỄN HLV "${name}"?\nTài khoản đăng nhập và hồ sơ sẽ bị mất hoàn toàn!`)) return;
      try {
  ...
  // --- HẠ CẤP HLV (GIỮ PROFILE, XÓA AUTH) ---
  const handleDowngrade = async (id: string, name: string) => {
      if (!confirm(`HẠ CẤP: Bạn muốn chuyển HLV "${name}" xuống thành VÕ SINH?\n- Hồ sơ sẽ được giữ lại.\n- Tài khoản đăng nhập (Email/MK) sẽ bị XÓA VĨNH VIỄN.`)) return;
      try {
  ...
  return (
    <div className="p-6 h-full bg-[#fdfbf7] overflow-y-auto custom-scrollbar">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-red-900/10 pb-4 gap-4">
        <h2 className="text-3xl font-serif font-bold text-red-900 uppercase tracking-wide">Quản Lý Giảng Viên</h2>
        <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative group w-full md:w-64">
                <input type="text" placeholder="Tìm tên, chức vụ, CLB..." className="w-full pl-10 pr-4 py-2 rounded-full border border-stone-200 bg-white focus:border-red-800 focus:ring-1 focus:ring-red-800 outline-none text-sm transition-all shadow-sm group-hover:border-red-300 font-sans" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 absolute left-3 top-2.5 text-stone-400 group-hover:text-red-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            </div>
            {isAdmin && <button onClick={openAddModal} className="bg-red-900 text-yellow-50 px-5 py-2 rounded-xl shadow-md font-bold hover:bg-red-800 flex items-center gap-2 transition-transform active:scale-95 text-sm whitespace-nowrap"><span>+</span> Thêm HLV</button>}
        </div>
      </div>
          ...
          return (
            <div key={coach.id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 p-5 flex gap-5 group border border-stone-100 hover:border-red-100 relative overflow-hidden">
              ...
              <div className="flex-1 flex flex-col justify-center min-w-0">
                <h3 className="font-serif font-bold text-xl text-stone-800 truncate group-hover:text-red-900 transition-colors mb-1">{coach.full_name}</h3>
                <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider bg-red-50 text-red-800 px-2 py-0.5 rounded-md border border-red-100">{coach.club_role || coach.title || 'HLV'}</span>
                ...
                <div className="text-sm text-stone-500 space-y-1">
                  <p className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-stone-300"></span>Đai: <span className="font-bold text-stone-700">{coach.belt_level}/22</span></p>
                  {masterName ? (<p className="flex items-center gap-1 truncate" title={`Sư phụ: ${masterName}`}><span className="w-2 h-2 rounded-full bg-yellow-500"></span>SP: <span className="font-bold text-stone-700">{masterName}</span></p>) : (<p className="flex items-center gap-1 truncate text-stone-300 italic"><span className="w-2 h-2 rounded-full bg-stone-200"></span>Chưa có sư phụ</p>)}
                  {coach.club_id && <p className="flex items-center gap-1 truncate" title={clubs.find(c => c.id === coach.club_id)?.name}><span className="w-2 h-2 rounded-full bg-blue-300"></span>{clubs.find(c => c.id === coach.club_id)?.name || 'Chưa rõ CLB'}</p>}
                </div>
              </div>
              ...
              {isAdmin && (
                <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                    ...
                    {/* Nút Hạ Cấp (Mới) */}
                    <button onClick={() => handleDowngrade(coach.id, coach.full_name)} className="bg-yellow-50 text-yellow-600 w-8 h-8 flex items-center justify-center rounded-full hover:bg-yellow-500 hover:text-white shadow transition-colors border border-yellow-200" title="Hạ cấp xuống Võ sinh">⬇</button>
                    
                    {/* Nút Xóa */}
                    <button onClick={() => handleDelete(coach.id, coach.full_name)} className="bg-red-50 text-red-600 w-8 h-8 flex items-center justify-center rounded-full hover:bg-red-600 hover:text-white shadow transition-colors" title="Xóa vĩnh viễn">×</button>
                </div>
              )}
            </div>
          );
      ...
      {/* --- CÁC MODAL GIỮ NGUYÊN (Code Crop/Add giống cũ) --- */}
      {/* ... (Phần Modal Crop và Add giữ nguyên như phiên bản trước) ... */}
      {showCropModal && (
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-[#fdfbf7] rounded-3xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border-4 border-double border-red-900/20">
            <div className="p-6 border-b border-red-100 flex justify-between items-center bg-white/50">
                <h3 className="text-2xl font-serif font-bold text-red-900 uppercase tracking-wide">{isEditing ? 'Cập Nhật Hồ Sơ' : 'Thêm Giảng Viên'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-12 gap-8">
              <div className="md:col-span-4 flex flex-col items-center justify-start pt-2">
                  <label className="cursor-pointer group relative block w-40 h-40 border-2 border-dashed border-stone-300 rounded-2xl hover:border-red-500 transition-colors bg-white overflow-hidden shadow-inner">
                      {formData.avatar_url ? <img src={formData.avatar_url} className="w-full h-full object-cover"/> : <div className="flex flex-col items-center justify-center h-full text-stone-300"><span className="text-4xl font-light">+</span></div>}
              ...
              <div className="md:col-span-8 space-y-5">
                  ...
                  <div className="bg-red-50/50 p-4 rounded-xl border border-red-100 space-y-4">
                      <div>
                         <label className="block text-xs font-bold text-red-800 mb-1 uppercase">Sư Phụ (Người trực tiếp dạy)</label>
                         <select className="w-full border border-red-100 p-2 rounded-lg focus:border-red-800 outline-none bg-white" value={formData.master_id} onChange={e => setFormData({...formData, master_id: e.target.value})}>
                            <option value="">-- Chọn Sư Phụ --</option>
                            {coaches.filter(c => c.id !== editId).map(c => (<option key={c.id} value={c.id}>{c.full_name} (Đai {c.belt_level})</option>))}
                         </select>
                      </div>
              ...
              <div className="col-span-1 md:col-span-12 flex justify-end gap-3 pt-6 border-t border-dashed border-stone-200">
                ...
                <button type="submit" disabled={loading || uploading} className="px-8 py-2 bg-red-900 text-yellow-50 rounded-xl hover:bg-red-800 font-bold shadow-lg transition-transform active:scale-95">{loading ? 'Đang Xử Lý...' : (isEditing ? 'Lưu Hồ Sơ' : 'Tạo Hồ Sơ')}</button>
              </div>
            </form>
          </div>
        </div>
