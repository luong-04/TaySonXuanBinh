...
export default function CoachManager({ userRole }: { userRole: string }) {
  ...
  const openEditModal = (coach: Coach) => {
      ...
      setFormData({
          ...
          join_date: coach.join_date ? String(coach.join_date).substring(0, 10) : '',
          dob: coach.dob ? String(coach.dob).substring(0, 10) : '',
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 md:mb-8 border-b border-red-900/10 pb-4 gap-4">
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
            <div className="relative group w-full md:w-64">
                <input type="text" placeholder="Tìm tên, chức vụ, CLB..." className="text-red-900 placeholder:text-red-700/50 w-full pl-10 pr-4 py-2 rounded-full border border-stone-200 bg-white focus:border-red-800 outline-none text-sm transition-all shadow-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      ...
      {showCropModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl animate-in zoom-in duration-300 flex flex-col h-[500px]">
            ...
            <div className="text-red-900 mt-4 px-2"><input type="range" value={zoom} min={1} max={3} step={0.1} onChange={(e) => setZoom(Number(e.target.value))} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer" /></div>
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8">
              ...
              <div className="col-span-1 md:col-span-8 space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div><label className="block text-xs font-bold text-stone-500 mb-1 uppercase">Email (*)</label><input required type="email" className="text-red-900 w-full border border-stone-200 p-3 rounded-xl focus:border-red-800 outline-none bg-white shadow-sm" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} /></div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
